...
out=$(curl -w "@curl-format.txt" -o benhmarks.txt -X GET "http://localhost:8080/api/v1/events?eventType=event&service=ADMINISTRATION" -H "accept: application/json" -H "Authorization: Bearer $token")
echo $out
